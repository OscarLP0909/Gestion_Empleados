...
export function validateNif(req: Request, res: Response, next: NextFunction): void {
...
function checkControlLetter(nif: string): boolean {
...
function checkCompanyControlDigit(nif: string): boolean {
    const digits = nif.substring(1, 8);
    const controlChar = nif.charAt(8);

    const sum =
        parseInt(digits.charAt(0)) * 2 +
